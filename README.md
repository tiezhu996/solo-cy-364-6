...
- **库存报损：仅店长可提交本店报损单（数量+原因，门店取自登录态）→ 总部/管理员审批，每张单据仅处理一次；总部/管理员不开放提交、只负责审批；通过自动扣减库存并生成损耗出库记录（库存不足审批失败、单据保持待审核），驳回必填原因、库存不变；店长仅看本店、总部/管理员看全部门店**
...
| POST | /api/v1/loss-orders | 店长提交本店报损单（门店取自登录态，无需传 store_id） | 仅店长，严格限流 |
